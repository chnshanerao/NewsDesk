...
def answer(conn, question: str, limit: int = 8) -> dict:
    ...
    rows = conn.execute(
        ...
        "JOIN clusters c ON c.id=cl.cluster_id ORDER BY c.last_ts DESC LIMIT 2000")
    ...
    for row in rows:
        ...
        refs = [dict(x) for x in conn.execute(
            ...
            "published_ts,quote,quote_field,quote_start,quote_end,quote_hash,similarity "
    ...
    return {
        ...
        "query_terms": terms,
        "findings": findings, "citation_coverage": 1.0 if findings else 0.0,
        "limitations": [
