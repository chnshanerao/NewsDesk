...
| 里程碑 | 状态 | 可验证结果 |
| --- | --- | --- |
| 全球信源 | 阶段完成 | 59 个启用信源、48 个 owner，覆盖中、英、葡语；28 条 AI 专线含 13 个一次源和 15 个独立/专业采编源 |
| 事件与证据 | 完成 | Schema v8 持久化 1,614 个 claims 与 1,941 条引用；引用覆盖率 100%，高优事件 claim 覆盖率 100% |
...
| AI / 科技雷达 | P0 完成 | 六赛道、28 条专线、近 72 小时 342 个 AI 事件，一次源/媒体跟进/独立印证分开展示 |
| 引用研究工作台 | P1 阶段完成 | 抽取式逐句引用研究答案；每条 finding 保留原文、来源角色、字符位置和 quote hash，无引用不输出 |
...
质量门禁也可通过 `/api/quality` 获取；当前实测为 37 pass / 7 warn / 0 fail，87/87 自动化测试通过。`healthy_with_warnings` 表示硬性门禁通过，但 AI 一次源长期目标、国际稿占比、owner 集中度、独立印证率、7 天行情历史和真实人工金标仍有未达目标项。
