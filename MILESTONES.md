...
| 里程碑 | 状态 | 可验证结果 |
| --- | --- | --- |
| 全球信源 | 阶段完成 | 74 个启用信源、61 个 owner、7 个地区；36 条 AI 专线含 21 个一次源、18 个近 30 天活跃一次源和 15 个独立/专业采编源；已补入联合早报、韩联社、共同社、ANSA、Rappler、VnExpress 等非大陆通讯社 |
| 事件与证据 | 完成 | Schema v10 起为每条引用保存 support/refute/unknown 关系、置信度和方法；6,929 条引用切片与 hash 全量校验，孤儿引用为 0；高优事件关系分类覆盖 100%，全库 95.3% |
...
| AI / 科技雷达 | P0 完成 | 六赛道、36 条专线、近 72 小时 478 个 AI 事件，一次源/媒体跟进/独立印证分开展示 |
| 详情页正文预览 | 完成 | 头条稿原文前 4 段、900 字双上限入库（`items.body`），累计尝试 1,300 篇成功 1,132 篇（87%），首页前 100 条事件覆盖 89%；摘要为空的源优先抽取，联合早报 40 篇里 32 篇拿到正文；抽不到退回 feed 摘要并注明来源，始终保留原文链接 |
...
质量门禁也可通过 `/api/quality` 获取；2026-09-07 实测为 46 pass / 6 warn / 0 fail，108/108 自动化测试通过。`healthy_with_warnings` 表示硬性门禁通过，但国际稿占比（0.15，目标 0.20）、最大 owner 集中度（0.43，目标 ≤0.35，主因中新网 7 条 feed）、独立印证率、7 天行情历史和真实人工金标仍有未达目标项。
